...
class FlashLB(EplbPolicy): 
    ...
    def __init__(self, config: DynamicConfig):
        ...
        self.buffer_expert_layer_num = \
            config.buffer_expert_layer_num if hasattr(config, "buffer_expert_layer_num") else 4
    ...
    def register_hotness(self, deployment, rank_load, num_layer, num_expert): 
        for layer in range(num_layer): 
            ...
            FlashLB.hotness_window[layer].append(hotness) 
    ...
    def rebalance_experts(self, current_expert_table, expert_workload):
        ...
        expert_workload = expert_workload.numpy()
        expert_workload +=1 
        num_layer, num_rank, replicas_per_rank = expert_workload.shape[0],expert_workload.shape[1],expert_workload.shape[2]
        num_replicas = num_rank * replicas_per_rank
        num_expert = np.unique(current_expert_table[0].reshape(-1)).shape[0]

        current_deployment = current_expert_table.numpy() 
