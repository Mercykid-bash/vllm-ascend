...
def select_experts(hidden_states: torch.Tensor,
    ...
    if mix_placement:
        pad_shared_expert_id = torch.full((topk_ids.shape[0],1), num_logical_experts, dtype=topk_ids.dtype, device=topk_ids.device)
        pad_shared_expert_weight = torch.full((topk_weights.shape[0],1), routed_scaling_factor, dtype=topk_weights.dtype, device=topk_weights.device)
        topk_ids = torch.cat([topk_ids, pad_shared_expert_id], dim=1)
        topk_weights = torch.cat([topk_weights,pad_shared_expert_weight], dim=1)
